...
public class ProtostuffJsonLineProvider<T extends Message> implements MessageBodyReader<Collection<T>>, MessageBodyWriter<Collection<T>> {
	...
	public void writeTo(Collection<T> t, Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType, MultivaluedMap<String, Object> httpHeaders,
		...
		output.tail = output.sink.writeByteArray(END, 0, END.length, output, output.tail);
