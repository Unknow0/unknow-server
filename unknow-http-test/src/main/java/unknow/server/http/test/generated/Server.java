...
import unknow.server.http.test.Error;
import unknow.server.http.test.Servlet;
...
final class Server extends NIOServerCli implements HttpRawProcessor {
	...
	private final EventManager createEventManager() {
		return new EventManager(new ArrayList<>(0), new ArrayList<>(0), new ArrayList<>(0), new ArrayList<>(0), new ArrayList<>(0), new ArrayList<>(0), new ArrayList<>(0));
	}

	private final ServletManager createServletManager() {
		Error s0 = new Error();
		WebserviceServlet s1 = new WebserviceServlet();
		Servlet s2 = new Servlet();
		DefaultServlet s3 = new DefaultServlet(new ArrayMap<>(new String[] { "/404.html", "/nioserver.properties" }, new Resource[] { new Resource(1618069378466L, 106L), new Resource(1636726958993L, 42L) }));
	...
	private final ServletContextImpl createContext() {
		return new ServletContextImpl("", new ArrayMap<>(new String[] {}, new String[] {}), SERVLETS, EVENTS, new NoSessionFactory(), new ArrayMap<>(new String[] {}, new String[] {}), new ArrayMap<>(new String[] { "7z", "aac", "avi", "bmp", "bz", "bz2", "css", "csv", "gif", "htm", "html", "ico", "ics", "jar", "jpeg", "jpg", "js", "json", "mid", "midi", "mpeg", "oga", "ogv", "ogx", "otf", "pdf", "png", "rar", "rtf", "sh", "svg", "tar", "tif", "tiff", "ts", "ttf", "wav", "weba", "webm", "webp", "woff", "woff2", "xhtml", "xml", "zip" }, new String[] { "application/x-7z-compressed", "audio/aac", "video/x-msvideo", "image/bmp", "application/x-bzip", "application/x-bzip2", "text/css", "text/csc", "image/gif", "text/html", "text/html", "image/x-icon", "text/calendar", "application/java-archive", "image/jpeg", "image/jpeg", "application/javascript", "application/json", "audio/midi", "audio/midi", "video/mpeg", "audio/ogg", "video/ogg", "application/ogg", "font/otf", "application/pdf", "image/png", "application/x-rar-compressed", "application/rtf", "application/x-sh", "image/svg+xml", "application/x-tar", "image/tiff", "image/tiff", "application/typescript", "font/ttf", "audio/x-wav", "audio/webm", "video/webm", "image/webp", "font/woff", "font/woff2", "application/xhtml+xml", "application/xml", "application/zip" }));
	}
	...
	private final void initialize() throws ServletException {
		...
		s[0].init(new ServletConfigImpl("error", CTX, new ArrayMap<>(new String[] {}, new String[] {})));
		s[1].init(new ServletConfigImpl("Webservice", CTX, new ArrayMap<>(new String[] {}, new String[] {})));
		s[3].init(new ServletConfigImpl("default", CTX, new ArrayMap<>(new String[] {}, new String[] {})));
		s[2].init(new ServletConfigImpl("test", CTX, new ArrayMap<>(new String[] { "content" }, new String[] { "it works" })));
	}
