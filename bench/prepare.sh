...
VER=$(wget -q -O - https://dlcdn.apache.org/tomcat/tomcat-10/ | grep '<a href="v9' | sed 's:.*>v\(10[^</]*\)/</a.*:\1:' | sort -rV | head -1)
mkdir -p tomcat
wget -O - https://dlcdn.apache.org/tomcat/tomcat-10/v$VER/bin/apache-tomcat-$VER.tar.gz | tar xz --strip-components=1 -C tomcat
rm -r tomcat/webapps/*
